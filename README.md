...
| `ws` | ワークスペースセレクター。fzfでプロジェクトを選び、新しいTerminal.appウィンドウでtmux+nvimを起動。プロジェクトごとにステータスバーの色が自動で変わる |

`ws` は `~/devel/` 直下のディレクトリを自動検出する。深い階層のプロジェクトは `aliases.sh` の `extra_projects` に直接登録。

tmux ステータスバーはプロジェクト名のハッシュから8色のダークカラーパレットで色分けされる。同じプロジェクトは常に同じ色になるため、複数ウィンドウを並行して開いても一目で区別できる。
